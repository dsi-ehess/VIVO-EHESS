            ...
            <#list group.classes as class>
                <#if (class.uri?contains("FacultyMember")) >
                    ...
                    <#if (class.individualCount > 0) >
                        <script>
                            var facultyMemberCount = ${class.individualCount?c};
                        </script>
                    <#else>
...
<#macro allClassGroups classGroups>
    ...
    <#else>
        ...
        <#else>
            <p>${i18n().please} <a href="/loginExternalAuthReturn" title="${i18n().login_to_manage_site}">${i18n().log_in}</a> ${i18n().to_manage_content}</p>
        </#if>
    </#if>
