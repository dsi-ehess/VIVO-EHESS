...
<div class="section-width bg-faculty">
            ...
            <#list group.classes as class>
                <#if (class.uri?contains("FacultyMember")) >
                    ...
                    <#if (class.individualCount > 0) >
                        <script>var facultyMemberCount = ${class.individualCount?string["0"]?replace(",","")?replace(".","")?replace(" ","")};</script>
                    <#else>
...
<#-- Loop through classGroups first so we can account for situations when all class groups are empty -->
    ...
    <#assign classGroupList>
        <div class="section-width bg-stats">
    ...
    <#else>
        ...
        <#if user.loggedIn>
            <#if user.hasSiteAdminAccess>
                <p>${i18n().you_can} <a href="${urls.siteAdmin}"
                                        title="${i18n().add_content_manage_site}">${i18n().add_content_manage_site}</a> ${i18n().from_site_admin_page}
                </p>
            </#if>
        <#else>
            <p>${i18n().please} <a href="${urls.login}"
                                   title="${i18n().login_to_manage_site}">${i18n().log_in}</a> ${i18n().to_manage_content}
            </p>
        </#if>
    </#if>
...
<#macro researchClasses classGroups=vClassGroups>
    <#assign foundClassGroup = false />
<div class="section-width bg-research">
    <section id="home-research" class="home-sections">
        ...
        <ul>
            ...
            <#if !foundClassGroup>
                <p>
                <li style="padding-left:1.2em">${i18n().no_research_content_found}</li></p>
            </#if>
        </ul>
    </section>
</div>
...
<div class="section-width bg-departments">
...
<#if academicDeptDG?has_content>
    ...
    <#-- HACK EHESS -->
        <#list selectLocale.locales as locale>
            <#if locale.selected>
                <#assign currentLocale = locale.code[0..1] />
            </#if>
        </#list>
        <#if currentLocale??>
            <#assign label = resultRow["name_${currentLocale}"]/>
        <#else>
            <#assign label = resultRow["name_fr"]/>
        </#if>

    <#-- HACK EHESS END -->
        {"uri": "${uri?url}", "name": "${label}${currentLocale}"}<#if (resultRow_has_next)>,</#if>
    </#list>
</#if>
    ];
